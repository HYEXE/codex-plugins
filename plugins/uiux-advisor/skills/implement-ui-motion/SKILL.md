...
description: 기존 프론트엔드에서 상태 전환 동작 자체가 주된 결과물인 모션과 마이크로인터랙션을 설계하고 CSS, Web Animations API, View Transition API, Anime.js, Motion 또는 GSAP 중 가장 단순한 기술로 구현·검증한다. 사용자가 애니메이션 적용, 버튼·카드의 동작 피드백, 페이지·레이아웃 전환, 스크롤·타임라인·SVG 모션, 라이브러리 선택, reduced-motion 또는 모션 성능 수정을 요청할 때 사용한다. 화면 구성이나 registry 컴포넌트 조합이 주된 결과물이면 compose-creative-ui를, 모션 원칙 설명이나 명세만 필요하면 uiux-advisor를, 기존 결과의 감사만 필요하면 uiux-auditor를 사용한다.
---
